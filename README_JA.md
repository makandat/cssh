...
- ウィンドウタイトルには、”cssh v0.2.n” と表示する。(n はビルドバージョン)
...
## 3.2 cd
- パラメータとしてディレクトリが指定してある場合、カレントディレクトリをそこへ移動する。
- パラメータがない場合は、ホームディレクトリ ( C:\Users\<user>) へ移動する。
...
## 3.4 cat (Alias type)
- テキストファイル (UTF-8のみサポート) の内容を表示する。オプション -n を付けると行番号も表示する。
...
## 3.7 cp (Alias copy)
- ファイルをコピーする。コピー先がディレクトリの場合、その下に新しいファイルを作ってコピーする。
- すでに同じ名前のファイルがある場合は上書きするが、そのファイルがReadOnlyの場合も -f オプションを付ければ上書きできる。
...
### 4.1.1 編集とコマンド入力 (Rev.3)
- 通常モードで edit コマンドを実行すると、画面最下行のコマンド入力行にカーソルを移動し、もし編集バッファに内容が残っていれば、その内容を表示する。編集バッファが空の場合は、何も表示しない。
- 画面の一番下の行は、コマンド入力行で編集モードのコマンド入力に使う。
- コマンド入力はプロンプト ‘> ‘ に続けて行い、Enter キーを押すとそのコマンドを実行する。
- quit コマンドの場合は通常画面に戻るが、それ以外のコマンドでは編集画面に留まる。
- コマンド入力をキャンセルするには、ESCキーを押す。
- 編集コマンド実行が失敗した場合は、エラーメッセージを最終行に表示する。これは、ESC キーを押すことでクリアされ、プロンプトと入力したコマンドを再び表示する。その際、プロンプトのカーソルはコマンドの最後に移動する。
- Rev 2 では編集そのものも、この画面で行えるようにすることになっていたが、処理が複雑になるので、編集そのものは外部エディタを使用するように変更する。(Rev 3)

### 4.1.2 外部エディタ連携 (Rev 3)
- 使用する外部エディタの検知は以下の順序とする。つまり notepad++ がインストールされていれば、notepad++ を外部エディタとして使い、そうでなければ notepad を外部エディタとする。(将来は設定ファイルで外部エディタを直接指定可能にする)
 first: notepad++
 second: notepad
- notepad++ の検知方法は標準インストールの場所で判別する。
- (旧)「編集領域」は単に現在の編集バッファを表示するだけにする。
- 「編集領域」での編集はできないが、スクロール等は可能とする。
- np (NotePad の意味) コマンドを編集モードコマンドとして追加する。
- np コマンドは編集バッファの内容を一時ファイルに保存し、外部エディタを起動する。
- ユーザは編集後、内容を一時ファイルに保存し外部エディタを閉じる。
- 外部エディタを閉じた後、一時ファイルの内容を予備編集バッファ にコピーし、それを画面に表示する。
- 以下、編集バッファは main buffer、 予備編集バッファは backup buffer と呼ぶことがある。
-  backup buffer と main buffer とは同じ形式だが別のメモリ領域とする。
- backup buffer の内容で main bufferの内容を置き換えてよいかの許可をユーザに確認する。(例) confirm ? (y/n) > 
- ユーザが許可した場合、backup buffer の内容と main buffer の内容を交換する。
-  main buffer の内容を表示して、main buffer の内容が更新された旨のメッセージを表示する。
- undo (編集モードコマンドとして新規追加) コマンドを実行すると、再び、 main buffer とbackup buffer の内容を交換する。(1回のみ可能)


##  4.2 編集モードの組み込みコマンド (Rev 3)
...
### 4.2.3 quit (Alias q)
- 編集モードを終了し、通常モードへ戻る。
- 編集バッファの内容は消去せず、次回、編集モードに戻った時は、表示状態も含めそのまま残っているものとする。(編集バッファ用一時ファイルに保存)
...
- 編集バッファの内容がファイル未保存の場合、スクリプトかデータの判別がつかないので、確認メッセージを表示する。(例）スクリプトとして実行してよいですか？(y/n) ← OS の UI 言語に従う
...
- cssh 標準関数を利用する場合は、#r <path>, using static ScriptStd をスクリプトの最初に挿入すること。
(例)
```
#r "C:/workspace/Cssh/cssh.App/bin/Debug/net10.0/cssh.Std.dll"
using static ScriptStd;

println("Hello, World!");
```

### 4.2.6 np (Rev.3, np は NotePad の意味)
- 外部エディタを起動する。外部エディタは可能なら notepad++、そうでないなら notepad とする。
- 編集バッファの内容を一時ファイル($TEMP/cssh_edit_yyyymmss_hhmmss.tmp)に保存し外部エディタを起動する。
- 編集バッファの永続化は不要。
- 外部エディタでの編集が終わったら、一時ファイルの内容を backup buffer へコピーして表示する。
- ユーザはその表示を見て正式なものにするか（編集バッファの内容として採用）判別する。
- 許可があったときは、編集バッファの内容と backup buffer の内容を交換して、編集バッファの内容を表示する。（メッセージをプロンプト領域に表示)
-メッセージは ESC キーで消去可能で、元のプロンプト表示に戻る。(UI仕様)

### 4.2.7 undo (Rev.3)
- backup buffer の内容を main buffer と交換した後、undo コマンドにより１度だけ元に戻すことができる。

### 4.2.8 /string (Rev.3)
- vim のように ‘/string’ とすると、編集バッファの先頭から検索を行い、最初に見つかった位置(行番号)を最下行に表示する。このメッセージは '/' (次の位置を検索) または ESC キーでクリアできる。
 (例) 行番号 n （/ または ESC キーを押してください)
- ２回目からは / のみで次の string を見つける。
- もし、string が見つからない場合、画面の最下行にメッセージを表示する。
- これは、ESCキーでクリアされ元のプロンプトを表示する。
...
- run コマンドでスクリプトを起動する。(dotnet-script コマンドの実行)
- C#スクリプト完全互換のスクリプトのみサポート。
